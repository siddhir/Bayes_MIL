...
import h5py
import yaml
from models.model_bmil import probabilistic_MIL_Bayes_vis
...
def infer_single_slide(model, features, label, reverse_label_dict, k=1, bayes_args=None):
	...
	with torch.no_grad():
		...
		elif isinstance(model, (probabilistic_MIL_Bayes_vis)):
			if bayes_args:
				out_prob = 0
				out_atten = 0
				out_logits = 0
		...
		if isinstance(model, (probabilistic_MIL_Bayes_vis)):
