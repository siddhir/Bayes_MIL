...
import h5py
import yaml
from models.model_bmil import probabilistic_MIL_Bayes_vis, probabilistic_MIL_Bayes_enc
...
def infer_single_slide(model, features, label, reverse_label_dict, k=1, bayes_args=None):
	...
	with torch.no_grad():
		...
		elif isinstance(model, (probabilistic_MIL_Bayes_vis, probabilistic_MIL_Bayes_enc)):
			if bayes_args:
				out_prob = 0
				out_atten = 0
				out_logits = 0
				...
				for _ in range(N_SAMPLES):
					logits, Y_prob, Y_hat, _, A = model(features, validation=True)
					out_prob += Y_prob
					out_atten += A.detach()
					out_logits += logits
				...
				out_atten_save = out_atten.cpu().clone().detach()
		...
		if isinstance(model, (probabilistic_MIL_Bayes_vis, probabilistic_MIL_Bayes_enc)):
			print('Y_hat: {}, Y: {}, Y_prob: {}'.format(reverse_label_dict[Y_hat],
														label,
														["{:.4f}".format(p) for p in out_prob.cpu().flatten()]))

			probs, ids = torch.topk(out_prob, k)
			...
			preds_str = np.array([reverse_label_dict[idx] for idx in ids])
	if isinstance(model, (probabilistic_MIL_Bayes_vis, probabilistic_MIL_Bayes_enc)):
		return ids, preds_str, probs, out_atten_save.cpu().numpy(), vis_data.view(-1, 1).cpu().numpy(), vis_total.view(-1, 1).cpu().numpy(), vis_model.view(-1, 1).cpu().numpy()
	else:
...
if __name__ == '__main__':
	...
	if model_args.initiate_fn == 'initiate_model':
		if model_args.model_type.startswith('bmil'):
			model, bayes_args = initiate_model(model_args, ckpt_path)
		else:
	...
	for i in range(len(process_stack)):
		try:
			...
			if model_args.model_type.startswith('bmil'):
				Y_hats, Y_hats_str, Y_probs, A, vis_data, vis_total, vis_model = infer_single_slide(model, features, label, reverse_label_dict, exp_args.n_classes, bayes_args=bayes_args)
			else:
			...
			if not os.path.isfile(block_map_save_path):
				...
				coords = file['coords'][:]
				file.close()
				asset_dict = {'attention_scores': A, 'data_unc': vis_data, 'total_unc': vis_total, 'model_unc': vis_model,'coords': coords}
			...
			scores = dset[:]
			coords = coord_dset[:]
			data_uncs = data_unc_dset[:]
			total_uncs = total_unc_dset[:]
			model_uncs = model_unc_dset[:]
			file.close()
			...
			#
			# 	# uncs = []
			# 	# uncs.extend(data_uncs)
			# 	# uncs.extend(total_uncs)
			# 	# uncs.extend(model_uncs)
			# 	# uncs = np.array(uncs)
			#
			# 	heatmap_data_uncs = drawHeatmap(data_uncs, coords, slide_path, uncs_type=0, wsi_object=wsi_object, cmap=heatmap_args.cmap,
			# 						  alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1,
			# 						  blank_canvas=False,
			# 						  thresh=-1, patch_size=vis_patch_size, convert_to_percentiles=False)
			#
			# 	heatmap_data_uncs.save(os.path.join(r_slide_save_dir, '{}_blockmap_data_uncs.png'.format(slide_id)))
			#
			# 	heatmap_total_uncs = drawHeatmap(total_uncs, coords, slide_path, uncs_type=1, wsi_object=wsi_object,
			# 									cmap=heatmap_args.cmap,
			# 									alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1,
			# 									blank_canvas=False,
			# 									thresh=-1, patch_size=vis_patch_size, convert_to_percentiles=False)
			#
			# 	heatmap_total_uncs.save(os.path.join(r_slide_save_dir, '{}_blockmap_total_uncs.png'.format(slide_id)))
			#
			# 	heatmap_model_uncs = drawHeatmap(model_uncs, coords, slide_path, uncs_type=2, wsi_object=wsi_object,
			# 									 cmap=heatmap_args.cmap,
			# 									 alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1,
			# 									 blank_canvas=False,
			# 									 thresh=-1, patch_size=vis_patch_size, convert_to_percentiles=False)
			#
			# 	heatmap_model_uncs.save(os.path.join(r_slide_save_dir, '{}_blockmap_model_uncs.png'.format(slide_id)))
			#
			# 	del heatmap
			# 	del heatmap_data_uncs
			# 	del heatmap_total_uncs
			# 	del heatmap_model_uncs
